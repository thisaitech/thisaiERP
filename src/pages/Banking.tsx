      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">{t.banking.bankingAndFinance}</h1>
              <p className="text-sm text-muted-foreground">{t.banking.manageAccountsCashCheques}</p>
            </div>
          </div>
          <motion.button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-muted-foreground text-xs">{t.banking.availableBalance}</p>
          </div>
          <p className="text-2xl font-bold text-foreground">₹{availableBalance.toLocaleString()}</p>
          <p className="text-xs text-muted-foreground mt-1">{t.banking.bankPlusCashMinusPending}</p>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2">
            <Wallet size={18} weight="duotone" className="text-primary" />
            <p className="text-muted-foreground text-xs">{t.banking.cashInHand}</p>
          </div>
          <p className="text-2xl font-bold text-foreground">₹{accounts.cashInHand.balance.toLocaleString()}</p>
          <p className="text-xs text-muted-foreground mt-1">{t.banking.availableCash}</p>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-muted-foreground text-xs">{t.banking.pendingCheques}</p>
          </div>
          <p className="text-2xl font-bold text-foreground">₹{pendingCheques.toLocaleString()}</p>
          <p className="text-xs text-muted-foreground mt-1">{accounts.cheques.filter(ch => ch.status === 'pending').length} {t.banking.cheques}</p>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2">
            <TrendUp size={18} weight="duotone" className={netWorth >= 0 ? "text-success" : "text-destructive"} />
            <p className="text-muted-foreground text-xs">{t.banking.netWorth}</p>
          </div>
          <p className={cn(
            "text-2xl font-bold",
          ...
          <p className="text-xs text-muted-foreground mt-1">{t.banking.assetsMinusLoans} ({accounts.loans.length} {t.banking.active})</p>
        </motion.div>
      </div>

      <div>
        {/* Tabs */}
        <div className="bg-card rounded-lg shadow-lg border border-border mb-6 lg:mb-8">
          <div className="flex overflow-x-auto">
            {[
              ...
              <button
                ...
                className={cn(
                  "flex items-center gap-2 px-4 py-2.5 lg:px-6 lg:py-3 text-sm lg:text-base font-medium whitespace-nowrap transition-all border-b-2",
                  selectedTab === tab.id
                    ? "border-primary text-primary"
                    : "border-transparent text-muted-foreground hover:text-foreground"
                )}
              >
                <tab.icon size={18} weight={selectedTab === tab.id ? "duotone" : "regular"} className="lg:w-5 lg:h-5" />
                {tab.label}
              </button>
            ))}
          </div>
        ...
        {selectedTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            {/* Bank Accounts */}
            <div className="bg-card rounded-lg shadow-lg border border-border p-4 lg:p-6">
              ...
              <div className="space-y-3">
                {accounts.bankAccounts.map((account, index) => (
                  <motion.div
                    ...
                    className="p-4 bg-gradient-to-br from-primary/5 to-accent/5 rounded-lg border border-primary/10 hover:border-primary/30 transition-all"
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={cn(
                        ...
                        account.type === 'current' ? "bg-primary/10 text-primary" : "bg-success/10 text-success"
                      )}>
                        {account.type}
                      </span>
                    </div>
            ...
            <div className="bg-card rounded-lg shadow-lg border border-border p-6">
              ...
              <div className="p-6 bg-gradient-to-br from-success/5 to-success/10 rounded-lg border border-success/20 text-center">
                <Money size={48} weight="duotone" className="text-success mx-auto mb-4" />
                <p className="text-3xl font-bold mb-2">₹{accounts.cashInHand.balance.toLocaleString()}</p>
                <p className="text-sm text-muted-foreground mb-4">{t.banking.availableCash}</p>
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-4 py-2 bg-success text-white rounded-lg text-sm font-medium hover:bg-success/90"
                  >
                    {t.banking.cashIn}
                  </button>
                  <button
              ...
              {/* Recent Transactions */}
              <div className="mt-4">
                <p className="text-sm font-medium mb-2">{t.banking.recentTransactions}</p>
                <div className="space-y-2">
                  {transactions.filter(t => t.account === 'Cash in Hand').slice(0, 3).map((txn, index) => (
                    <div key={txn.id} className="flex items-center justify-between p-2 bg-muted/50 rounded">
                      <div>
